...
if __name__ == "__main__":
    ...
    while True:
        ...
        minutely_market_code_i_have = [
            m[:-4] for m in upbit_minutely_files if m.endswith("arr")
        ]
        daily_market_code_i_have = [
            m[:-4] for m in upbit_daily_files if m.endswith("arr")
        ]

        if set(market_code_in_upbit) != set(minutely_market_code_i_have):
            print("업비트 종목들과 저장된 종목들이 서로 다름")
            new_code = set(market_code_in_upbit) - set(minutely_market_code_i_have)
            delete_code = set(minutely_market_code_i_have) - set(market_code_in_upbit)
        ...
        if now.hour == 9:
            ...
            while True:
                ...
                if now.hour == 9 and now.minute == 30 and (0 <= now.second < 10):
                    ...
                    # 이후 데이터 백업하기
                    backup_start(
                        DIR_UPBIT_DAILY_CANDLE, DIR_UPBIT_DAILY_CANDLE_BACKUP, ".arr"
                    )
                    backup_start(
                        DIR_UPBIT_MINUTELY_CANDLE,
                        DIR_UPBIT_MINUTELY_CANDLE_BACKUP,
                        ".arr",
                    )
